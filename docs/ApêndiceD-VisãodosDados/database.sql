...
-- functions
CREATE OR REPLACE FUNCTION notify_assist() RETURNS TRIGGER AS $$
DECLARE payload JSON;
BEGIN IF (TG_OP = 'DELETE') THEN payload := row_to_json(OLD);
ELSE payload := row_to_json(NEW);
END IF;
PERFORM pg_notify(
...
INSERT
    OR
UPDATE
    OR DELETE ON assist FOR EACH ROW EXECUTE FUNCTION notify_assist();
-- -- Insert base data
